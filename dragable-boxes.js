...
function createRSSBoxesFromCookie() 
   { 
	...
	{				
		if(cookieCounter==0){
			createARSSBox('visimisi',2,false,8,1);			
		...
		var titleOfNewBox = 'Visi and Mission';
		...
		var htmlContentOfNewBox = '<DIV> System : Keico HRMS</div>';	// HTML content of new box
		var htmlContentOfNewBox1 = '<DIV> Version : STATo2A01</div>';	// HTML content of new box
		var htmlContentOfNewBox2 = '<DIV> Database: MySQL Server</div>';	// HTML content of new box
		var htmlContentOfNewBox3 = '<DIV> Authorized for:<?php echo $companyname; ?></div>';	// HTML content of new box
		var titleOfNewBox = 'System Information';
		if(!staticObjectArray['staticObject2']){	// The box is not stored in cookie - we need to create it.
			...
			document.getElementById('dragableBoxContent' + newIndex).innerHTML = htmlContentOfNewBox + "\n" + htmlContentOfNewBox1 + "\n" + htmlContentOfNewBox2 + "\n" + htmlContentOfNewBox3;		
			document.getElementById('dragableBoxHeader_txt' + newIndex).innerHTML = titleOfNewBox;		
		}else{	// Box is stored in cookie - all we have to do is to move content into it.
